<div class="template-study">
  <h4>📚 Study Notes</h4>
  <p><strong>Subject:</strong> {{ $memory->tags ?? 'General' }}</p>
  <pre class="bg-light p-3 rounded">{{ $memory->content }}</pre>
  ...
  @if($memory->file_path)
    ...
    <div class="mt-2">
      @if(in_array($ext, ['jpg','jpeg','png','gif','webp']))
        <img src="{{ asset('storage/'.$memory->file_path) }}" class="img-fluid rounded shadow" alt="study image">
